...
tab_code_Gen_metal<- source("external/tab_Gen_metal.R", local = TRUE)$value
...
# Import Data ----
df_AU_Basins <-read.csv("Data/AU_Basins.csv")
df_AU_Data <- read.csv("Data/AU_Data.csv")

df_all_AUs <- df_AU_Basins %>%
  select(AU_ID) %>% 
  distinct()%>% 
  mutate(InGIS = "Yes") %>% 
  arrange(AU_ID)

AU_list <- df_all_AUs[,1]
